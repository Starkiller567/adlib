...
static _attr_unused _attr_always_inline
dstr_t _dstr_from_number32_helper(uint32_t uval, bool is_signed, unsigned int base,
				  bool leading_zeros, bool sign_always, bool uppercase)
{
	...
	switch (base) {
	case 2:
		n = leading_zeros ? 32 : ilog2(uval) + 1;
		break;
	case 8:
		n = leading_zeros ? 11 : ilog2(uval) / 3 + 1;
		break;
	case 10:
		n = leading_zeros ? 10 : ilog10(uval) + 1;
		break;
	case 16:
		n = leading_zeros ? 8 : ilog2(uval) / 4 + 1;
		if (uppercase) {
	...
	dstr_t dstr = dstr_new();
	dstr_append_uninitialized(&dstr, n + (sign_char ? 1 : 0));
	char *p = dstr;
	if (sign_char) {
		p[0] = sign_char;
		p++;
...
static _attr_unused dstr_t _dstr_from_number32(uint32_t uval, unsigned int flags, bool is_signed)
{
	if (likely(flags == DSTR_FMT_DEFAULT)) {
		flags |= DSTR_FMT_DECIMAL;
	}
	unsigned int base = flags & __DSTR_FMT_BASE_MASK;
	bool leading_zeros = unlikely(flags & DSTR_FMT_LEADING_ZEROS);
	bool sign_always = unlikely(flags & DSTR_FMT_PLUS_SIGN);
	bool uppercase = unlikely(flags & DSTR_FMT_UPPERCASE);
	if (likely(base == 10)) {
		return _dstr_from_number32_helper(uval, is_signed, 10, leading_zeros,
						  sign_always, uppercase);
	}
	if (likely(base == 16)) {
		return _dstr_from_number32_helper(uval, is_signed, 16, leading_zeros,
						  sign_always, uppercase);
	}
	if (base == 2) {
		return _dstr_from_number32_helper(uval, is_signed, 2, leading_zeros,
						  sign_always, uppercase);
	}
	if (base == 8) {
		return _dstr_from_number32_helper(uval, is_signed, 8, leading_zeros,
						  sign_always, uppercase);
	}
	assert(false);
	return NULL;
...
static _attr_unused _attr_always_inline
dstr_t _dstr_from_number64_helper(uint64_t uval, bool is_signed, unsigned int base,
				  bool leading_zeros, bool sign_always, bool uppercase)
{
	int64_t sval = uval;
	char sign_char = 0;
	...
	switch (base) {
	case 2:
		n = leading_zeros ? 64 : ilog2(uval) + 1;
		break;
	case 8:
		n = leading_zeros ? 22 : ilog2(uval) / 3 + 1;
		break;
	case 10:
		n = leading_zeros ? (is_signed ? 19 : 20) : ilog10(uval) + 1;
		break;
	case 16:
		n = leading_zeros ? 16 : ilog2(uval) / 4 + 1;
		if (uppercase) {
			alphabet = "0123456789ABCDEF";
		}
		break;
...
static _attr_unused dstr_t _dstr_from_number64(uint64_t uval, unsigned int flags, bool is_signed)
{
	if (likely(flags == DSTR_FMT_DEFAULT)) {
		flags |= DSTR_FMT_DECIMAL;
	}
	...
	bool uppercase = unlikely(flags & DSTR_FMT_UPPERCASE);
	if (likely(base == 10)) {
		return _dstr_from_number64_helper(uval, is_signed, 10, leading_zeros,
						  sign_always, uppercase);
	}
	if (likely(base == 16)) {
		return _dstr_from_number64_helper(uval, is_signed, 16, leading_zeros,
						  sign_always, uppercase);
	}
	if (base == 2) {
		return _dstr_from_number64_helper(uval, is_signed, 2, leading_zeros,
						  sign_always, uppercase);
	}
	if (base == 8) {
		return _dstr_from_number64_helper(uval, is_signed, 8, leading_zeros,
						  sign_always, uppercase);
	}
	assert(false);
	return NULL;
...
#define __DSTR_FOREACH_INTTYPE(f)		\
	f(int, int)				\
	f(uint, unsigned int)			\
	f(long, long)				\
...
#define __DSTR_FROM_NUMBER_FUNC(name, type)				\
	__AD_LINKAGE dstr_t dstr_from_##name(type val, unsigned int flags) \
	{								\
		return sizeof(type) <= 4 ? _dstr_from_number32(val, flags, (type)-1 < 0) : \
			_dstr_from_number64(val, flags, (type)-1 < 0);	\
	}
__DSTR_FOREACH_INTTYPE(__DSTR_FROM_NUMBER_FUNC)
#undef __DSTR_FROM_NUMBER_FUNC
#undef __DSTR_FOREACH_INTTYPE
