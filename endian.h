...
typedef uint16_t _be16;
typedef uint32_t _be32;
typedef uint64_t _be64;

typedef uint16_t _le16;
typedef uint32_t _le32;
typedef uint64_t _le64;

static inline _be16 cpu_to_be16(uint16_t u)
{
	return is_big_endian() ? u : bswap16(u);
}

static inline _be32 cpu_to_be32(uint32_t u)
{
	return is_big_endian() ? u : bswap32(u);
}

static inline _be64 cpu_to_be64(uint64_t u)
{
	return is_big_endian() ? u : bswap64(u);
}

static inline uint16_t be16_to_cpu(_be16 b)
{
	return is_big_endian() ? b : bswap16(b);
}

static inline uint32_t be32_to_cpu(_be32 b)
{
	return is_big_endian() ? b : bswap32(b);
}

static inline uint64_t be64_to_cpu(_be64 b)
{
	return is_big_endian() ? b : bswap64(b);
}

static inline _le16 cpu_to_le16(uint16_t u)
{
	return is_little_endian() ? u : bswap16(u);
}

static inline _le32 cpu_to_le32(uint32_t u)
{
	return is_little_endian() ? u : bswap32(u);
}

static inline _le64 cpu_to_le64(uint64_t u)
{
	return is_little_endian() ? u : bswap64(u);
}

static inline uint16_t le16_to_cpu(_le16 l)
{
	return is_little_endian() ? l : bswap16(l);
}

static inline uint32_t le32_to_cpu(_le32 l)
{
	return is_little_endian() ? l : bswap32(l);
}

static inline uint64_t le64_to_cpu(_le64 l)
{
